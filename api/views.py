...
class Disc_type_list(APIView):
    # Get all discussions which type is Either Article or Question
...
class Get_search_Disc_title(APIView):
    # Get all discussions which title matches searched term example: /api/discussion/list/?title=Sports
    def get(self, request, format=None):
        lis=['title']
...
class Disc_search_by_title_text_disctype(APIView):
    # Discussion should be searched by title, text and discussion_type    
    def get(self, request, format=None):
...
class Create_comment(APIView):
    ...
    # Add comment api notes:only logged in user can add comment one user can not add more than 1 
...
def comment_create(request):
    if request.user.is_authenticated():
        ...
        if comment == True:
            ...
            if res in result:
                return Response({'error': 'Sorry !! you can not comment on your own Discussion.'}, status=status.HTTP_200_OK)
            else:
                serializer = CommentSerializer(data=request.data)
                if serializer.is_valid():
                    serializer.save()
...
def DiscussionListSearchAll(request):
    ...
    if lis == lis2:    
        ...
        return Response({'error': 'Please enter title,text,discussion_type in URL, same as like mention in error.'}, status=status.HTTP_200_OK)
